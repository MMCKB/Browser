...
| 地址与搜索 | 支持 HTTP(S) 地址、常见域名与可选搜索引擎。搜索设置仅显示“搜索引擎”；向右滑动可进入自定义名称与搜索地址输入。切换立即更新首页而不重建设置页；非编辑状态下地址栏在搜索结果页显示本次关键词、在其他网页显示网页标题，聚焦编辑时则显示可修改的关键词或完整网址；底部地址栏会随输入法上移。 |
| 首页 UI | 设置入口统一命名为“首页 UI 编辑”，其独立页面顶部提供实时预览；以“内容 / 文字 / 搜索框”分类展示对应设置。可选择搜索引擎名或自定义标题，配置标题字号、加粗、斜体与调色盘颜色，并以精致 0–100% 滑杆调整搜索框圆角、不透明度和模糊强度。相关输入与调色盘使用统一的圆角磨砂弹窗。应用信息入口统一命名为“应用信息”。 |
| Tab 工具 | 统一三行三列工具栏：添加/移除书签、下载管理、分享、复制网址、清缓存、回首页、刷新、电脑/移动模式切换和设置。电脑模式会应用桌面 User-Agent 并刷新当前网页。 |
